...
-- Lets a signed-in user check whether THEIR OWN id is in this table —
-- this is how the admin panel verifies "am I actually an admin" after
-- login. Does not expose anyone else's row.
drop policy if exists "self read own admin row" on public.admins;
create policy "self read own admin row" on public.admins for select using (id = auth.uid());
...
drop policy if exists "public read resume_items" on public.resume_items;
create policy "public read resume_items" on public.resume_items for select using (true);
drop policy if exists "public read resume_files" on public.resume_files;
...
drop policy if exists "admin write content_blocks" on public.content_blocks;
...
create policy "admin write nav_links" on public.nav_links for all using (is_admin()) with check (is_admin());
...
drop policy if exists "admin write resume_items" on public.resume_items;
create policy "admin write resume_items" on public.resume_items for all using (is_admin()) with check (is_admin());
drop policy if exists "admin write resume_files" on public.resume_files;
...
insert into public.content_blocks (page, section, key, value) values
  ...
  ('home','hero','spline_url','https://my.spline.design/boxeshover-1zjS3MSNd4kulP7aZNJbuZkL/'),
  ...
  ('home','about','photo_url','https://images.unsplash.com/photo-1500648767791-00dcc994a43e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=200'),
  ...
  ('home','contact','email','hello@harshkapadiya.dev'),
